...
import { Suspense, useState, useMemo, useCallback, useEffect, lazy } from "react";
...
const RevenueItemFormModal = lazy(() =>
  import("./RevenueItemFormModal").then((m) => ({ default: m.RevenueItemFormModal }))
);
...
function RevenueItemsInner({
  ...
  }, [bulkLoading, selectedIds, updateItem]);
  ...
  }, [bulkLoading, selectedIds, deleteItems]);
          ...
          {selectedIds.size > 0 && (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-[5px] rounded-[7px] border border-[#e5e7eb] bg-white px-3 py-1.5 text-[12px] font-medium text-[#6b7280] transition-colors hover:border-[#c5e49a] hover:text-[#3d6b14] disabled:opacity-50"
              >
                ✅ Mark collected
              </button>
              <button
                ...
                disabled={bulkLoading}
                className="flex items-center gap-[5px] rounded-[7px] border border-[#e5e7eb] bg-white px-3 py-1.5 text-[12px] font-medium text-[#6b7280] transition-colors hover:border-[#fecaca] hover:text-[#ef4444] disabled:opacity-50"
              >
                🗑 Delete
              </button>
            </div>
